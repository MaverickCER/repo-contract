...
**Migration.** Every record committed under this ADR's original (pre-amendment) model (68 as of
the commit that landed this amendment) was backfilled with a single migration pass: `verifiedBy:
...
authoring fields. **This is explicitly a one-time baseline import, not an individual, independent
fresh review of each one** — a later reader must not read the identical verifier/timestamp across
that whole set as evidence any of them were re-examined at migration time; none were. The
content-binding is what makes the baseline meaningful going forward regardless: any backfilled
record whose prose is edited from here on immediately loses its inherited sign-off and must be
...
- **Skipping the backfill entirely and letting every pre-existing suppression fail until
