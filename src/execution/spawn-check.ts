...
function buildEnv(check: CheckDefinition, ambientEnv: NodeJS.ProcessEnv): Record<string, string> {
  ...
  if (check.inheritEnv !== false) {
    for (const [key, value] of Object.entries(ambientEnv)) {
      // `NodeJS.ProcessEnv`'s index signature is typed `string | undefined`,
      // but a real environment map never actually produces an `undefined`
      // value here for an own enumerable key -- assigning `undefined` to an
      // env var coerces it to the literal string `"undefined"` (confirmed
      // empirically against `process.env`). Kept only to satisfy that type,
      // not because it changes observed behavior.
      // Stryker disable next-line ConditionalExpression -- NodeJS.ProcessEnv's index signature is typed string | undefined, but a real environment map never actually produces an undefined value here for an own enumerable key; kept only to satisfy that type, not because it changes observed behavior.
