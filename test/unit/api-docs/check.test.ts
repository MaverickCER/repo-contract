...
describe("runApiDocsCheck (real repository files)", () => {
  it("finds every committed report up to date with the real public surface", async () => {
    const evidence = await runApiDocsCheck(REPO_ROOT)
  ...
  it("finds no undocumented symbol in any committed report", async () => {
    const evidence = await runApiDocsCheck(REPO_ROOT)
    expect(evidence.reports.length).toBeGreaterThan(0)
    expect(evidence.reports.flatMap((report) => report.undocumentedMarkers)).toEqual([])
  })

  it("covers both the root and presets entry points", async () => {
    const evidence = await runApiDocsCheck(REPO_ROOT)
